...
export default function ContactForm({ contactFormState, onFormClose, onEmailSentSuccess, onEmailSentFailure }) {
  ...
  async function handleAPICall() {
    try {
      ...
      if (res.status !== 200) {
        ...
        handleOnEmailSentFailure();
      } else {
        handleOnEmailSentSuccess();
      }
    } catch (error) {
  ...
  /**
   * Handler for when the email is sent successfully
   */
  ...
  /**
   * Handler for when the email is failed to send
   */
  const handleOnEmailSentFailure = () => {
    if (onEmailSentFailure) onEmailSentFailure();
  }
    ...
    <AnimatePresence>
      {
        contactFormState && (
          ...
          className="fixed z-10 grid h-full w-full over place-items-center p-4 sm:p-0 backdrop-blur-sm sm:m-0"
